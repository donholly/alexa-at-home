    ...
    this.playPlaylist = function(playlistName, res){
    	...
    	var queueAndPlay = function (playlistNum, trackNum) {
    		...
    		mopidy.playlists.getPlaylists().then(function (playlists) {
    			...
    			console.log("Playlists: " + JSON.stringify(playlistNames));

    			var playlist;

    			if (playlistName != null) {
    				console.log("Attempting to match on playlist name '" + playlistName + "'")
    				var found = false;
    				for (var index in playlists) {
    					...
    					if (pl["name"].toLowerCase() == playlistName.toLowerCase()) {
    						playlist = pl;
    						console.log("MATCH FOUND ("+playlistName+") == ", playlist.name);
    						found = true;
    					}
    				}
    			}
